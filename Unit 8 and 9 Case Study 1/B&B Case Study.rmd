...
### We can see the Colorado, California, Michigan, Oregon and Texas have the highest number of breweries
```{r}
...
### IBU has 1005 missing values 
### Style has 5 missing values 
### Our initial approach was to scrape the web to find the missing values for ABV & IBU. However we encountered quite a few challenges in extracting the data from the interner
### due to inconsistencies in HTML formatting across different websites. Our final approach was to calculate the median values of ABV and IBU and impute them into the missing values
```{r}
...
maxibuplot<- maxibuperstate %>% ggplot(aes(x = reorder(State,-IBU), y = IBU)) + 
  ...
  xlab('State') + 
  geom_text(aes(40, 138, label="Oregon \n Highest Overall"))
...
### this could be due to the number of outliers present in the dataset
```{r}
...
abvibuplot<-brewerydata %>% ggplot(aes(x= IBU, y = `% ABV`, color = `% ABV`)) + 
  geom_point() + 
  ylim(2.5, 13) + 
  theme_dark() + 
...
```{r}
# Perform statistical analysis on data to gather inference 
# We will check for significant differences in IBU among all states
names(brewerydata)[4]= 'ABV'
dunn_test_IBU_results = dunnTest(IBU~State,data=brewerydata,method='bh')
dunn_test_IBU_results$res[dunn_test_IBU_results$res[4] < 0.05,]

# We will check for significant differences in ABV among all states 
dunn_test_ABV_results = dunnTest(ABV~State,data=brewerydata,method='bh')
dunn_test_ABV_results$res[dunn_test_ABV_results$res[4] < 0.05,1]
```
